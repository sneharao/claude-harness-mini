...
|---|---|---|
| `000_design` | One-time project bootstrap: problem statement, entities, ubiquitous language, architecture shape, stack pick | Cagan (is this worth building?), Evans (domain modelling, bounded contexts), Cockburn (architecture boundaries from day one), Chase + Taylor (only if the project is an AI/LLM product) |
...
| `003_2_review_local` | Same five-pass review, run locally before pushing | Same persona panel as `003_1_review_pr`. |
