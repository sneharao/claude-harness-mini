...
- **Next.js App Router (`persona-nextjs.md`)** — Hex/onion-ready split. Folders are kebab-case (`src/features/create-user/`). The slice lives in `src/features/<feature>/` with `command.ts` (Zod), `handler.ts` (logic; first line `import "server-only";`), `view.tsx` (RSC by default), plus slice-local `.tsx` sub-components as flat files. `src/app/` is transport-only: `app/<feature>/page.tsx` is a thin RSC that calls the handler, `app/<feature>/actions.ts` is a `"use server"` shim, `app/api/<feature>/route.ts` is a REST shim. Pages Router is out of scope. See the persona for the full hex graduation story.
